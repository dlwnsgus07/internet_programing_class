...
<body>
	...
	<div
		...
		<!-- form을 통해 사용자에게 제목과 썸네일, 영상파일을 등록받고 작성자를 등록 받은후 register_do.jsp로 정보들을 Post방식으로 보내주어 register_do에서 해당 정보들을 처리해줄 수 있도록 함 -->
		<form action="register_do.jsp" enctype="multipart/form-data" method="POST">
			<div class="register_title row">
				<label for="V_title" class="col-3 r_label">제목</label>
				 <input type="text" name="title" class="video_title col-9"
					placeholder="영상 제목을 입력하세요">
			</div>
			<div class="register_thumbnail row">
				<label for="V_title" class="col-3 r_label">썸네일
					등록</label> <input type="file" name="thumbnail"
					class="video_thumbnail col-9">
			</div>
			<div class="register_video row">
				<label for="video" class="col-3 r_label">영상파일 등록</label>
				<input type="file" name="video" class="video col-9">
			</div>
			<div class="register_info row">
				<label for="info" class="col-3 r_label">영상 설명</label>
				<textarea name="info" class="info col-9"></textarea>
			</div>
			<div class="register_author row">
			<label for="author" class="col-3 r_label">작성자</label>
				<input type = "text" name="author" class="col-9 author">
			</div>
			<div class="register_btn row">
				<button type="submit" class="register_submit btn col-3">등록</button>
				<button type="button" class="register_cancellation col-3 btn" onClick="location.href='videoBoard.jsp'">취소</button>
